...
class RecomendadorMusical:
    ...
    def analizar_emocion_y_recomendar(self, texto_usuario):
        ...
        try:
            ...
            # Usar Deezer solo para preview e imagen
            _, imagen, preview = self._buscar_deezer(
            ...
            link_youtube, miniatura = self._buscar_youtube(
                datos_cancion["titulo"],
                datos_cancion.get("artista")
            )

            # Si no hay imagen en Deezer, usar miniatura de YouTube
            if not imagen:
                imagen = miniatura

            return descripcion, link_youtube, imagen, preview, link_youtube
    ...
    def _crear_prompt(self, texto_usuario):
        ...
        Analiza la emoción del usuario y recomienda UNA canción que encaje perfectamente.
        Si el usuario menciona un artista o género musical, inclúyelo en el resultado.
        ...
        {{
            ...
            "genero": "género musical de la canción (si se menciona, usarlo)"
        }}

        Elige canciones conocidas y populares para asegurar disponibilidad.
        """
        ...
        try:
            respuesta = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "Eres un experto musicólogo que recomienda canciones basadas en análisis emocional. Responde SOLO con JSON válido."
                    },
    ...
    def _buscar_youtube(self, titulo, artista=None):
        query = f"{titulo} {artista}" if artista else titulo
        search_url = f"https://www.youtube.com/results?search_query={requests.utils.quote(query)}"
        # Miniatura usando formato estándar (puede variar según ID real)
        # Aquí solo devolvemos el enlace de búsqueda y no ID de video específico.
